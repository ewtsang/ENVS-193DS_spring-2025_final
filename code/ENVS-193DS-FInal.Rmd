...
ggplot(data = sst_clean, #making visualization using cleaned sst data 
  ...
  #setting the axi, comparing month (x axis) and mean monthly sea surface temperature (y axis)
  scale_color_manual(
    values = colorRampPalette(c("lightblue","darkblue"))(6)
  ) + 
  #setting colors of the plot in 6 separate blues
  geom_line(
    aes(group = year)) + 
  #group by
  geom_point(
...
ggplot(preds, aes(x = x, y = predicted, color = as.factor(group), fill = as.factor(group))) +
  ...
  labs(title = "Predicted Swift Parrot Nest Box Occupancy by Edge Distance and Season",
